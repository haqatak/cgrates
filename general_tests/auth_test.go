...
func TestAuthLoadCsvError(t *testing.T) {
	...
	chargerProfiles := ``
	csvr, err := engine.NewTpReader(dbAuth.DataDB(), config.CgrConfig().CacheCfg(), engine.NewStringCSVStorage(utils.CSVSep, destinations, timings, rates, destinationRates,
