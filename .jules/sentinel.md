...
## 2024-05-30 - Fix Insecure PRNG Fallback and math/rand usage
**Vulnerability:** The function `RandomInteger` in `utils/coreutils.go` fell back to generating random numbers using `math/rand.Int63n` if `crypto/rand` failed. Additionally, `RandomConverter` in `utils/dataconverter.go` used `math/rand` by default. This relied on a deterministic and predictable weak pseudo-random number generator (PRNG).
**Learning:** System source of randomness (like `crypto/rand`) can fail. However, silently falling back to a weak, predictable pseudo-random number generator gives a false sense of security. If an attacker can trigger this failure mode (e.g. file descriptor exhaustion), they can predict the output.
**Prevention:** PRNG functions meant to be secure must fail securely (e.g., panic or return an error) instead of falling back to a weak PRNG like `math/rand`. Additionally, always default to `crypto/rand` or wrapper functions that use it (like `utils.RandomInteger`) when generating randomized data elements in data conversion or manipulation routines.
