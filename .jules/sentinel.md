...
## 2026-07-02 - Replace insecure `math/rand` with `utils.RandomInteger` in `utils/dataconverter.go`
**Vulnerability:** The `RandomConverter` component inside `utils/dataconverter.go` used the deterministic pseudo-random number generator `math/rand`. Since outputs of `RandomConverter` could be used to generate critical items that require unpredictability (such as tokens or IDs), a weak PRNG sequence exposes the application to predictability attacks.
**Learning:** General-purpose random generators like `math/rand` should not be used in components whose output may serve security-critical processes (like authentication or session management). When building flexible data converters, it's safer to default to cryptographically secure alternatives.
**Prevention:** Default to using `crypto/rand` (or utilities that wrap it, like `utils.RandomInteger`) when writing general-purpose functions that output randomized data, and strictly reserve `math/rand` for internal testing, deterministic algorithms, or highly intensive performance use-cases where collision resistance is demonstrably not an issue.
