...
## 2024-05-18 - Replacing Insecure math/rand with Secure utils.RandomInteger
**Vulnerability:** Weak, non-cryptographic random numbers generated by `math/rand.Int()` were used in `utils/dataconverter.go` (inside `RandomConverter`). This makes the output predictable.
**Learning:** `math/rand` is not suitable for generating secure values. The internal utility `utils.RandomInteger` provides a secure alternative by using `crypto/rand` and failing securely if `crypto/rand` errors, by intentionally calling `panic(err)`. This codebase utilizes an unseeded or predictably seeded math/rand for security.
**Prevention:** Always use `crypto/rand` (or the project's secure wrapper `utils.RandomInteger(min, max)`) when generating random numbers, and avoid `math/rand` entirely, except inside highly concurrent non-security-critical load testing code (like `cmd/cgr-tester`), where cryptographic PRNG can cause overhead.
