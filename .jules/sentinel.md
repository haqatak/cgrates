...
## 2024-05-24 - Use of Weak Pseudo-Random Number Generator in DataConverter
**Vulnerability:** The `RandomConverter.Convert` function in `utils/dataconverter.go` used `math/rand.Int()` and `math/rand.Intn()` to generate random values. `math/rand` is a deterministic, predictable pseudo-random number generator that isn't cryptographically secure. Relying on it could allow attackers to predict outputs.
**Learning:** `math/rand` should not be used in critical components when cryptographic security is required. Additionally, falling back silently from `crypto/rand` to `math/rand` (as seen previously in `RandomInteger`) degrades security without warning.
**Prevention:** Always use `crypto/rand` when unpredictability is required, using `utils.RandomInteger(min, max)`. To fail securely, `RandomInteger` should panic rather than falling back if `crypto/rand` fails.
