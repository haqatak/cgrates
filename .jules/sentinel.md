...
## 2024-05-24 - [Fix SQL Injection in JSONB Queries]
**Vulnerability:** A SQL Injection vulnerability existed in `engine/storage_sql.go` where the `SQLImpl` interface methods (`extraFieldsExistsQry`, `extraFieldsValueQry`, etc.) were directly interpolating untrusted parameters into SQL strings via `fmt.Sprintf` rather than utilizing parameter binding.
**Learning:** Returning unparameterized SQL strings from database abstraction layers directly into GORM's `Where()` method prevents the ORM from sanitizing user inputs. When working with native PostgreSQL JSONB operators (e.g., `?`, `?|`), the GORM parameter placeholder `?` conflicts with the operator.
**Prevention:** Always refactor interface methods handling SQL logic to return a tuple of `(string, []any)` to bubble parameters up to the ORM query builder. When building parameterized JSONB queries in GORM for PostgreSQL, use `??` to correctly escape the native JSONB operator so GORM doesn't attempt to interpret it as a parameter placeholder (e.g., `" extra_fields ?? ?"`, `[]any{field}`).
