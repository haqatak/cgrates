...
## 2026-06-30 - Predictable PRNG Fallback and Insecure Usage in RandomConverter
**Vulnerability:** The `RandomInteger` function in `utils/coreutils.go` fell back to the weak PRNG `math/rand.Int63n` if `crypto/rand` failed, which could compromise the unpredictability of critical tokens or identifiers. Furthermore, `RandomConverter` in `utils/dataconverter.go` used `math/rand` directly, creating a predictable generation mechanism for its outputs.
**Learning:** Security-sensitive PRNG functions should not silently fall back to insecure PRNGs upon failure; instead, they should panic or error explicitly (fail securely). Furthermore, when converting generic integers for unpredictable contexts, cryptographically secure functions must always be enforced.
**Prevention:** Always use `crypto/rand` (or its secure wrappers like `RandomInteger`) exclusively when generating non-deterministic values. Remove any fallbacks to `math/rand` in these wrappers and replace them with `panic(err)` to ensure fail-secure behavior. Do not use `math/rand` in any data converters capable of being configured for secure contexts.
