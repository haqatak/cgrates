...
## 2026-06-28 - [Replace math/rand with Secure Crypto/Rand in RandomConverter]
**Vulnerability:** The `RandomConverter.Convert` function in `utils/dataconverter.go` relied on the weak `math/rand` package. The `rand.Int()` and `rand.Intn()` functions utilize deterministic pseudo-random number generators, allowing predictability if an attacker discerns the seed sequence.
**Learning:** For components that require unpredictable value generation, specifically converters handling potentially sensitive contexts, falling back on non-cryptographic PRNGs poses a security risk, particularly in generating secure tokens or unique session identifiers.
**Prevention:** Always favor generating random integers through a function backed by `crypto/rand`, such as `utils.RandomInteger`. Limit `math/rand` exclusively to scenarios requiring high-performance bulk generation without security constraints (like benchmark tests or specific non-critical randomized sorting logic).
