...
## 2024-05-30 - Replaced math/rand with cryptographically secure PRNG in RandomConverter
**Vulnerability:** The `RandomConverter.Convert` method in `utils/dataconverter.go` used `math/rand.Int()` and `math/rand.Intn()`, relying on a weak pseudo-random number generator (PRNG) which can generate predictable sequences if the seed is known.
**Learning:** Usage of `math/rand` should be avoided for generating tokens, session IDs, or any application logic where predictability could be exploited.
**Prevention:** Rely on cryptographically secure PRNGs such as `utils.RandomInteger` (which utilizes `crypto/rand` underneath) when generating numbers or strings intended for potentially security-sensitive contexts, and only use `math/rand` if strictly required for performance where cryptographic security is irrelevant.
