...
## 2025-05-29 - Secure Random Number Generation in DataConverter
**Vulnerability:** The `RandomConverter` in `utils/dataconverter.go` used `math/rand.Int()` and `math/rand.Intn()` to generate random integers. `math/rand` is a non-cryptographic pseudo-random number generator, meaning the sequences it produces are deterministic and predictable. If used for anything requiring collision resistance or security, an attacker could predict the generated outputs.
**Learning:** For general-purpose secure randomization across the application, always rely on `crypto/rand`. In this codebase, the wrapper `utils.RandomInteger` provides a convenient interface over `crypto/rand`.
**Prevention:** Avoid `math/rand` in converters or utility functions that might be used for generating sensitive tokens, identifiers, or applying security-critical randomness. Default to `utils.RandomInteger` where integer generation within a range is required.
