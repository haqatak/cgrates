...
## 2026-06-23 - Prevent Predictable Random Number Generation in  Fallback
**Vulnerability:** The function `RandomInteger` in `utils/coreutils.go` generated random numbers securely using `crypto/rand.Int`, but contained an insecure fallback mechanism that used `math_rand.Int63n` if the cryptographic generator failed. Furthermore, `utils/dataconverter.go` explicitly utilized the deterministic `math/rand.Int`.
**Learning:** Silently falling back to a weak pseudo-random number generator (PRNG) when a secure PRNG fails is a severe security risk, as it introduces predictable entropy without alerting the system of the failure. `math/rand` should never be used where unpredictability is important for security.
**Prevention:** Always use `crypto/rand` when generating numbers that require unpredictability. If `crypto/rand` fails, the application should `panic` rather than falling back to a deterministic PRNG, as a failure implies a severely compromised environment.
## 2024-05-30 - Prevent Predictable Random Number Generation in `math/rand` Fallback
**Vulnerability:** The function `RandomInteger` in `utils/coreutils.go` generated random numbers securely using `crypto/rand.Int`, but contained an insecure fallback mechanism that used `math_rand.Int63n` if the cryptographic generator failed. Furthermore, `utils/dataconverter.go` explicitly utilized the deterministic `math/rand.Int`.
**Learning:** Silently falling back to a weak pseudo-random number generator (PRNG) when a secure PRNG fails is a severe security risk, as it introduces predictable entropy without alerting the system of the failure. `math/rand` should never be used where unpredictability is important for security.
**Prevention:** Always use `crypto/rand` when generating numbers that require unpredictability. If `crypto/rand` fails, the application should `panic` rather than falling back to a deterministic PRNG, as a failure implies a severely compromised environment.
