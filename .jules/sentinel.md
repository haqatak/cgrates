...
## 2024-05-24 - Use of Weak Pseudo-Random Number Generator in RandomConverter
**Vulnerability:** The function `Convert` in `RandomConverter` (in `utils/dataconverter.go`) generated random numbers using `math/rand.Int()` and `math/rand.Intn(x)`. This weak pseudo-random number generator (PRNG) is predictable and deterministic. Since `RandomConverter` is a general-purpose data converter, these generated values could be used in security-sensitive contexts like session IDs, tokens, or backoffs where unpredictability is essential.
**Learning:** For general-purpose components that might be used for security purposes, always use cryptographically secure PRNGs. `math/rand` should only be used in highly performance-critical, non-security contexts (like `boolGen` for sorting) or when determinism is explicitly required (like in tests or benchmarks).
**Prevention:** Always use `crypto/rand` (or wrappers like `utils.RandomInteger` in this codebase) when generating numbers, strings, or bytes unless cryptographic security is explicitly not required and performance is a proven bottleneck.
