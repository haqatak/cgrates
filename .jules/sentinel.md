...
## 2026-06-14 - Secure PRNG Fallback Hardening
**Vulnerability:** In `utils/coreutils.go`, the `RandomInteger` function (which wraps `crypto/rand`) had a fallback mechanism that silently degraded to the cryptographically weak `math/rand` if reading from the system entropy pool failed.
**Learning:** Silently falling back to a weak pseudo-random number generator gives a false sense of security. If a secure random number is required, and the system cannot provide one (e.g., due to entropy pool exhaustion or OS failure), the application must fail securely (fail closed) rather than continue with predictable numbers.
**Prevention:** When wrapping cryptographically secure PRNGs like `crypto/rand`, always handle read errors by panicking or returning an error to the caller, effectively halting the insecure operation. Never silently substitute a weak PRNG like `math/rand` in its place.
